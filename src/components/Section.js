  ...
  return (
    <section>
      <header className="flex items-center justify-between mb-4">
        ...
        {
          more && (
            <NavLink className={"text-xs font-semibold uppercase text-link hover:underline tracking-wider"} to={more}>hepsini gör</NavLink>
          )
        }
      </header>
        ...
        {items.map(item => (
          <NavLink key={item.id} to="/" className={"bg-footer p-4 rounded hover:bg-active group"}>
            <div className="pt-[100%] relative mb-4">
              ...
              <button className="w-10 h-10 rounded-full bg-primary absolute bottom-2 right-2 group-hover:flex group-focus:flex items-center justify-center hidden">
